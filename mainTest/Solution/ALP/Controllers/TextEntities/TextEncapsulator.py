...
from ....ALP.Controllers.Normalization.Normalizer import *
from ....ALP.Controllers.Morphology.Compounding.CompoundParsing import CompoundParsing
from ....ALP.Controllers.Morphology.AffixParser import AffixParser
from ....ALP.Controllers.Morphology.MorphologicalAnalyzer import MorphologicalAnalyzer
from ....ALP.Controllers.Morphology.Entities.DerivedCliticless import DerivedCliticless
from ....ALP.Controllers.Tagging.OverdueTagging.OverdueTagger import OverdueTagger
from ....ALP.Controllers.Tagging.PrematureTagging.PrematureTagger import PrematureTagger
from ....ALP.Models.Lexicon.SpecialWordsRepository import SpecialWordsRepository
from ....ALP.Models.Lexicon.RootsAndPatternsRepository import RootsAndPatternsRepository
from ....ALP.Models.Lexicon.SpecialWords.ProperNoun import *
from ....ALP.Models.General.TransducersXmlLoader import *
...
class TextEncapsulator(object):
    ...
    def __init__(self, string=None):
        ...
        self.String = string
        # ????????????????????????????????????????????????????????????????????
        #        self.__RootsAndPatternsRepository = RootsAndPatternsRepository.RootsAndPatternsRepository()
        self.__RootsAndPatternsRepository = RootsAndPatternsRepository()

        # ????????????????????????????????????????????????????????????????????
        #        self.__SpecialWordsRepository = SpecialWordsRepository.SpecialWordsRepository()
    ...
    def LoadFromFiles(self, baseDirectory, rootsFolder, \
        ...
        if (baseDirectory != None):
            ...
            self.__MorphologicalAnalyzer.SetRepositories(self.__RootsAndPatternsRepository,
                                                         self.__SpecialWordsRepository)
        ...
        for si in range(len(self.Sentences)):
            for wi in range(len(self.Sentences[si].Words)):
                if (self.Sentences[si].Words[wi].TokenType.Id == TokenType.Constants.Id.ArabicText \
                    ...
                    subRootsAndStems = self.__MorphologicalAnalyzer.FillWithStemsAndRootsAcurateClitics(
                        self.Sentences[si].Words[wi])
    ...
    def RenderHtml(self, stream):
        ...
        .Tag{width:40% font-size:12pt font-family:Courier Newfont-weight:bold}\
        ...
        .diacritic{color:Red}\
        .Certainty{width: 8%font-size:12pt font-family:Courier Newfont-weight:bold}\
        ...
        for si in range(len(self.Sentences)):
            ...
            for wi in range(len(self.Sentences[si].Words)):
                if (self.Sentences[si].Words[wi].TokenType.Id != TokenType.Constants.Id.ArabicText):
                    continue
                originalString = ArabicStringUtility.ColorizeDiacraticInHtml(ArabicStringUtility,
                                                                             self.Sentences[si].Words[
                                                                                 wi].OriginalString)
                numberOfPossibilities = len(self.Sentences[si].Words[wi].SurfaceFormMorphemes)
                ...
                stream.write(''.join(['<tr class=title>', \
                                      ...
                                      '<td colspan=3>', 'المقاطع (عدد التراكيب المحتملة ', str(numberOfPossibilities),
                                      ')', '</td>', \
                                      '</tr>']))
                ...
                leftTableData = ''.join(
                    ['<td class=subtitle style="{font-size:24pt}" rowspan=', str(numberOfPossibilities), '>',
                     originalString, '</td>'])
    ...
    def RenderSurfaceFormMorphemesHtml(self, stream, surfaceFormMorphemes, leftTableData=''):
        ...
        voweled_form = ArabicStringUtility.ColorizeDiacraticInHtml(ArabicStringUtility,
                                                                   surfaceFormMorphemes.VoweledForm)
