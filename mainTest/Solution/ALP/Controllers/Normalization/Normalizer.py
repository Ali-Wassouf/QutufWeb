...
from ....ALP.Models.Normalization.NormalizationRulesDict import *
        ...
        for i in range(len(sentences)):
            for j in range(len(sentences[i].Words)):
                ...
                for key, value in FirstNormDict.items():
                    sentences[i].Words[j].FirstNormalizationForm = sentences[i].Words[j].FirstNormalizationForm.replace(
                        key, value)
                    if (updateBy == 1):
        ...
        for i in range(len(sentences)):
            for j in range(len(sentences[i].Words)):
                ...
                for key, value in SecondNormDict.items():
                    sentences[i].Words[j].SecondNormalizationForm = sentences[i].Words[
                        j].SecondNormalizationForm.replace(key, value)
                    if (updateBy == 2):
