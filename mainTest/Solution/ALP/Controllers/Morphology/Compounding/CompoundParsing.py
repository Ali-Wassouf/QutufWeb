...
'''
from .....ALP.Controllers.General.ArabicStringUtility import *
from .....ALP.Controllers.Tokenization.TokenType import TokenType
from .....ALP.Controllers.Morphology.Entities.SurfaceFormMorphemes import *
    ...
    def Parsing(self, textEncapsulator, specialWords):
        ...
        for sentence in textEncapsulator.Sentences[:]:
            ...
            while index < len(sentence.Words) - 2:
                ...
                unvoweldForm = ' '.join(
                    [sentence.Words[index].SecondNormalizationForm, sentence.Words[index + 2].SecondNormalizationForm])
                firstNormalizationForm = ' '.join(
                    [sentence.Words[index].FirstNormalizationForm, sentence.Words[index + 2].FirstNormalizationForm])

                if unvoweldForm in compoundNounsDict.keys():
                    compound = compoundNounsDict[unvoweldForm]
                    if (not ArabicStringUtility.IsCompatible(ArabicStringUtility, firstNormalizationForm,
                                                             compound.VoweledForm)):
                        index += 1
                        continue
                    originalString = ' '.join(
                        [sentence.Words[index].OriginalString, sentence.Words[index + 2].OriginalString])
                    ...
                    sentence.Words[index].SurfaceFormMorphemes.append(
                        SurfaceFormMorphemes([], compound.ReturnAsUnderivedCliticless(), []))
